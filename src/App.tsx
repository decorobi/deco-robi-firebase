...
export default function App() {
  ...
  const [newOrder, setNewOrder] = useState({
    order_number: '', customer: '', product_code: '', description: '',
    ml: '' as any, qty_requested: '' as any, steps_count: 0,
  });
  ...
  const onStart = async (row: any) => {
    setTimers((t) => ({ ...t, [row.id!]: { running: true, startedAt: Date.now(), elapsed: Number(row.elapsed_sec || 0) } }));
  ...
  const onPause = async (row: any) => {
    ...
    const extra = t.startedAt ? Math.round((Date.now() - t.startedAt) / 1000) : 0;
    const elapsed = (t.elapsed || 0) + extra;
    setTimers((tt) => ({ ...tt, [row.id!]: { running: false, startedAt: null, elapsed } }));
    await updateDoc(doc(db, 'order_items', row.id!), { status: 'pausato', elapsed_sec: elapsed, timer_start: null } as any);
    setOrders((prev) => prev.map((o: any) => (o.id === row.id ? { ...o, status: 'pausato', elapsed_sec: elapsed, timer_start: null } : o)) as any);
  };
  ...
  const confirmStop = async () => {
    ...
    if (!stopStep || stopStep < 1 || (stepsCount > 0 && stopStep > stepsCount)) { alert('Seleziona un passaggio valido.'); return; }
    if (!stopPieces || stopPieces <= 0) { alert('Inserisci il numero di pezzi (maggiore di 0).'); return; }
    if (!stopOperator) { alert('Seleziona un operatore.'); return; }

    const t = timers[row.id!];
    const extra = t?.startedAt ? Math.round((Date.now() - t.startedAt) / 1000) : 0;
    const spentSec = Math.max(0, (t?.elapsed || 0) + extra);
    ...
    if (stopNotes && stopNotes.trim()) {
      notesLog.push({ ts: new Date().toISOString(), operator: stopOperator || null, text: stopNotes.trim(), step: pass, pieces: Number(stopPieces || 0) });
    }
    ...
    await setDoc(doc(db, 'order_items', row.id!), {
      ...
      last_pieces: Number(stopPieces || 0), last_duration_sec: spentSec,
      notes_log: notesLog,
    } as any, { merge: true });
  ...
  const createOrder = async () => {
    ...
    setNewOrder({ order_number: '', customer: '', product_code: '', description: '', ml: '' as any, qty_requested: '' as any, steps_count: 0 });
  };

  /* ------------------- Avanzamento fasi ------------------- */
  const openAdvance = (row: any) => { setAdvanceTarget(row); setAdvancePhase('in_essiccazione'); setAdvancePacked(0); setAdvanceOpen(true); };
  const confirmAdvance = async () => {
    ...
    setOrders((prev) => prev.map((o: any) => (o.id === id ? { ...o, ...patch, status_changed_at: new Date() as any } : o)));
  ...
  const exportExcel = () => {
    ...
    const rows = exportBase.map((o: any) => {
      ...
      return {
        Ordine: o.order_number, Cliente: o.customer || '', Codice: o.product_code,
        Descrizione: (o as any).description || '', ML: o.ml ?? '',
    ...
    exportBase.forEach((o: any, idx: number) => {
      aggregateStepStats(o).forEach((s) => aggRows.push({ Ordine: o.order_number, Riga: String(o.product_code || idx + 1), Passaggio: s.step, Pezzi: s.pieces || 0, Tempo: secToHMS(s.timeSec || 0) }));
    });
  ...
  const completati = useMemo(() => {
    ...
    return baseFiltered
      ...
      .filter((o: any) => {
        const inRight = ['eseguito','in_essiccazione','in_imballaggio','pronti_consegna'].includes(o.status);
        const parziale = Number(o.qty_done || 0) > 0;
    ...
    <div style={{ padding: 16 }}>
          ...
          <table className="table" style={{ width: '100%' }}>
            <thead>
              <tr>
                ...
                <th>Q.ta rich.</th><th>Q.ta fatta</th><th>Rimanenti</th>
                <th>Passaggi</th><th>Timer</th><th>Azioni</th>
              </tr>
            </thead>
            <tbody>
      ...
      <Modal open={stopOpen} onClose={() => setStopOpen(false)} title="Concludi lavorazione">
        ...
        <div className="grid" style={{ display: 'grid', gap: 8 }}>
          ...
          <label>
            ...
            <input type="number" min={1} step={1} value={stopPieces} onChange={(e) => setStopPieces(Number(e.target.value || 0))} required />
          </label>
          <label>
            <div>Operatore *</div>
            <select value={stopOperator} onChange={(e) => setStopOperator(e.target.value)} required>
      ...
      <Modal open={newOrderOpen} onClose={() => setNewOrderOpen(false)} title="Inserisci Ordine">
        <div style={{ display: 'grid', gap: 8 }}>
          ...
          <label><div>Descrizione</div><input value={newOrder.description} onChange={(e) => setNewOrder({ ...newOrder, description: e.target.value })} /></label>
          <label><div>ML</div><input value={newOrder.ml as any} onChange={(e) => setNewOrder({ ...newOrder, ml: e.target.value })} /></label>
