...
export default function App(){
  ...
  const handleImportCSV = async (file: File)=>{
    const parsed = await new Promise<RowIn[]>((resolve,reject)=>{
      Papa.parse<RowIn>(file, { header:true, skipEmptyLines:true, complete: (res)=> resolve(res.data as any), error: reject })
    })
  ...
  // Timer
  useEffect(()=>{ const id=setInterval(()=>setTimers(t=>{const n={...t},now=Date.now(); for(const k of Object.keys(n)){const tm=n[k]; if(tm.running&&tm.startedAt) n[k]={...tm,elapsed:Math.floor((now-tm.startedAt)/1000)}} return n}),250); return()=>clearInterval(id)},[])
